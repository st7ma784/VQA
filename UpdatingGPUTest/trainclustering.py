...
from datasets.captionloader import myCocoCaptions,myCocoImages
...
def train_fn(folders,net,epochs=10, lr=0.005):
    ...
    for e in range(epochs): 
        data_loaders=getDataloaders(folders,net,e)
        ...
        for loader in data_loaders:
            ...
            #save model here. 
        if (e+1)%10 ==0:
            torch.save(net.state_dict(), "./data/models/model{}epoch{}.pt".format("VitUpdatingContrastive2",e))
def getDataloaders(folders,model,epoch):
    dataloaders=[]
    for folder in folders:
        ...
        if epoch%2==0:
            ...
            dataset4sort=myCocoImages(root = os.path.join(dataDir,dataType),
                        annFile = annFile,
                        )
        else:
        ...
        train_sampler2=getSortedSampler(dataset4sort,method,device,B,e=epoch)
...
LR=0.00005
train_fn(folders,model,epochs=100,lr=LR)
