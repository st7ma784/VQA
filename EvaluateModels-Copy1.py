...
class LSTM(nn.Module):
    def __init__(self, clip,nb_layers,max_length=77,teacher_forcing_ratio=0.5):   

        super().__init__()
        Pretrained=clip.state_dict()["token_embedding.weight"]
        self.embedding = nn.Embedding(num_embeddings=Pretrained.size(0), embedding_dim=clip.embed_dim,padding_idx=0)
        Pretrained=clip.state_dict()["token_embedding.weight"]
...
#         )
        self.max_ans_length=nb_layers
        self.hidden_size=clip.embed_dim
        self.output_size=Pretrained.size(0)
        self.teacher_forcing_ratio=0.5
        self.padValue=0
        self.nb_layers = nb_layers
        self.embedding_dim = clip.embed_dim
        self.lstm = nn.LSTM(
            input_size=self.embedding_dim,
            hidden_size=self.clip.embed_dim,
            num_layers=self.max_ans_length,
            batch_first=True,
        )
        # output layer which projects back to tag space
        self.hidden_to_tag = nn.Linear(self.clip.embed_dim, self.output_size)

    def forward(self,Image,packedQtensor):
        
        QText,lens_unpacked=torch.nn.utils.rnn.pad_packed_sequence(packedQtensor,total_length=self.clip.context_length,batch_first=True)      
        #h0 = self.clip.encode_text(QText.long()).float()
        maxlen,_=torch.max(lens_unpacked,dim=0)
        print("maxlen: {}".format(maxlen))
        x = self.clip.token_embedding(QText.long()).type(self.clip.dtype)  # [batch_size, n_ctx, d_model]

        x = x + self.clip.positional_embedding.type(self.clip.dtype)
        x = x.permute(1, 0, 2)  # NLD -> LND
        x = self.clip.transformer(x)[:self.max_ans_length] 
        h0 = x.permute(1, 0, 2) # LND -> NLD
        #h0 = self.clip.ln_final(x).float()
        print("hidden : {}".format(h0.size()))#7,maxlen,512
        ...
        c0 = x @ self.clip.visual.proj
        
        #c0 = self.clip.encode_image(Image).float()
        #h0 = h0 / h0.norm(dim=-1, keepdim=True)
        c0 = c0 / c0.norm(dim=-1, keepdim=True)
        hidden=(h0,c0[:self.max_ans_length])
        batch_size = lens_unpacked.size()
        X = self.embedding(QText)#.unsqueeze(1)
        #X=QText
        print(X.size())#5,77,512
        X=torch.nn.utils.rnn.pack_padded_sequence(X, lens_unpacked, batch_first=True, enforce_sorted=False)
#         X = torch.nn.utils.rnn.pack_padded_sequence(x, X_lengths, batch_first=True)
        X, _ = self.lstm(X, hidden)
        #X, _ = torch.nn.utils.rnn.pad_packed_sequence(X,total_length=self.context_length, batch_first=True)
...
#         # run through actual linear layer
        X = self.hidden_to_tag(X.data)
        # 4. Create softmax activations bc we're doing classification
        # Dim transformation: (batch_size * seq_len, nb_lstm_units) -> (batch_size, seq_len, nb_tags)
        X = F.log_softmax(X, dim=1)
        # I like to reshape for mental sanity so we're back to (batch_size, seq_len, nb_tags)
#         X = X.view(batch_size, total_length, self.output_size)
        return X
...
class psCLIPLSTM(CLIPLSTM):
    def __init__(self,clip,dropout_p=0.1,max_length=77,teacher_forcing_ratio=0.5,Batch=100):   
        ...
        self.lstm=nn.LSTM(input_size=1,hidden_size=self.hidden_size,num_layers=1,bidirectional=True)
    def forward(self, image_tensor, QTensor,encoder_hidden): 
        ...
        return self.lstm(QTensor,(encoder_hidden.float(),encoder_output))
def train(LSTM,data_loader,n_iters,optim):
    ...
    for i in range(n_iters):
        ...
        for images,Q,A,QID, in data:
            #torch.cuda.synchronize()
            image_input=images.float().cuda()
            
            Q,lens_unpacked=torch.nn.utils.rnn.pad_packed_sequence(Q,total_length=77,batch_first=True)      

            #QA=pack_sequence(A,enforce_sorted=False).cuda()
            encoder_hidden = LSTM.clip.encode_text(Q.cuda()).float()
            #encoder_hidden = encoder_hidden / encoder_hidden.norm(dim=-1, keepdim=True)
            #Q=pack_sequence(Q,enforce_sorted=False).cuda()#to(device,non_blocking=True)#Bx77
#             #image_input -= image_mean[:, None, None]
#             #image_input /= image_std[:, None, None]
#             #Bx767
...
#             #print(Answers)
            outputs=LSTM(image_input,Q.unsqueeze(-1),encoder_hidden)#.permute(1,0,2)#[77, B, 49408]) to B,77,V
            #outs=torch.zeros(outputs.size()).cuda()
            print(outputs.size())#B,AnsLen,V
            ...
            #    try:
            #        indexs=torch.arange(startindexes[i],outputs.size(0)).cuda()
            #        outputs[:,i]=F.pad(torch.index_select(outputs[:,i],0,indexs.long()),(0,0,0,startindexes[i]))
...
#     out[i,:]= torch.index_select(A[i], 0, indexs.long())
            ...
            loss=criterion(outputs[:A.data.size(0)],A.data)
        ...
        scheduler.step()
        torch.save(LSTM, "./data/models/CLOZELSTMs/CLIPpsLSTMv1e{}.pt".format(i))
...
def collate(batch):
    im,Q,A,qid = zip(*batch)
    im=torch.stack(im)
    Q=pack_sequence(Q,enforce_sorted=False).cpu() 
    A=pack_sequence(A,enforce_sorted=False).cpu() # output=16x max(len)#to(device,non_blocking=True)#Bx77
    return im,Q,A,qid
...
LSTMmodel=psCLIPLSTM(model,10)#.cuda()
...
optimizer=torch.optim.AdamW([p for p in LSTMmodel.parameters() if p.requires_grad], lr=0.001,eps= 1e-3)
train(LSTMmodel,dataloader, 2,optimizer) 
torch.save(LSTMmodel, "./CLIPpsLSTMv1.pt")
